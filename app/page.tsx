...
// ============================================================
// DONNÉES DE SECOURS PREMIUM
// ============================================================
const BACKUP_OBJECTIVES: Objective[] = [
  {
    id: '1',
...
const BACKUP_COLLABORATORS: Collaborator[] = [
  {
    id: 'u1',
    first_name: 'Irina',
    last_name: 'VIEYRA',
...
const BACKUP_ACTIVITIES: Activity[] = [
  {
    id: 'a1',
    user_id: 'u1',
    description: 'Irina VIEYRA a audité la matrice contractuelle du projet de souveraineté.',
    date: '2026-06-10',
    type: 'audit',
  },
  {
    id: 'a2',
    user_id: 'u2',
    description: 'Nouveau livrable de diagnostic financier initial créé pour un entrepreneur béninois.',
    date: '2026-06-09',
    type: 'creation',
  },
  {
    id: 'a3',
    user_id: 'u2',
    description: 'Serge KODJO a mis à jour les dossiers existants du Cabinet DOUKE.',
    date: '2026-06-08',
    type: 'update',
  },
];

const BACKUP_REALISATIONS: Realisation[] = [
  {
    id: 'r1',
    user_id: 'u2',
    objective_id: '1',
    description: 'Finalisation de la structure du modèle Excel - onglet hypothèses complété',
    date: '2026-06-09',
    duration_hours: 3,
    progress_after: 65,
    validated_by: null,
  },
  {
    ...
    user_id: 'u3',
    objective_id: '3',
    description: 'Réunion de cadrage avec les équipes terrain de Parakou — 12 participants',
    date: '2026-06-08',
    duration_hours: 2,
    progress_after: 45,
    validated_by: null,
...
const BACKUP_REMARQUES: Remarque[] = [
  {
    id: 'rem1',
    from_id: 'u2',
    to_id: 'u4',
...
// ============================================================
// CATÉGORIES — CORRECTION : une seule déclaration par tableau
// puis ALL_CATEGORIES = fusion des deux
// ============================================================
...
// ✅ CORRECTION PRINCIPALE : ALL_CATEGORIES déclarée une seule fois, dédupliquée par id
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-[#1E293B] border border-slate-700/50 rounded-2xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-300 transition-all">
          ...
          <div className="mt-6 pt-5 border-t border-slate-700/50">
            <p className="text-[9px] font-black text-slate-500 uppercase tracking-wider mb-3">Profils disponibles sur cette instance :</p>
            <div className="space-y-1.5">
              {AUTH_ACCOUNTS.map((acc) => (
                <div key={acc.id} className="flex items-center justify-between bg-[#0F172A] rounded-lg px-3 py-2 border border-slate-800">
                  <div className="flex items-center gap-2">
                    <span className="text-sm">{acc.emoji}</span>
                    <span className="text-[10px] font-black text-slate-300">{acc.username}</span>
                  </div>
                  <span className={`text-[9px] font-black px-2 py-0.5 rounded uppercase tracking-wider ${
                    acc.role === 'Super-Admin' ? 'bg-blue-500/20 text-blue-400'
                    : acc.role === 'Senior Analyst' ? 'bg-slate-500/20 text-slate-300'
                    : acc.role === 'Field Lead' ? 'bg-emerald-500/20 text-emerald-400'
        ...
        <p className="text-center text-[10px] text-slate-600 font-bold mt-4 uppercase tracking-wider">
          © 2026 Cabinet DOUKE — Accès restreint. Toute tentative non autorisée est tracée.
        </p>
      </div>
    </div>
  );
    ...
    <>
          ...
          <div key={idx} className={`bg-white p-6 rounded-2xl border shadow-sm flex flex-col justify-between bg-gradient-to-b ${card.bg} ${card.border}`}>
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className={`text-3xl font-black mt-2 tracking-tight ${card.alert ? 'text-rose-600' : 'text-slate-900'}`}>{card.value}</p>
              </div>
              <div className="bg-white p-2.5 rounded-xl shadow-sm border border-slate-100">{card.icon}</div>
            </div>
            <p className="text-[11px] text-slate-500 font-medium mt-4 pt-3 border-t border-slate-50">{card.sub}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
        ...
        <div className="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-9 gap-2">
          {ALL_CATEGORIES.filter((cat) => objectives.some((o) => o.category === cat.id)).map((cat) => {
            const count = objectives.filter((o) => o.category === cat.id).length;
            return (
              <div key={cat.id} className={`p-3 rounded-xl border text-center cursor-pointer transition-all hover:scale-105 ${CATEGORY_STYLES[cat.id]}`}
                onClick={() => setCurrentView('Objectifs')}>
                <div className="text-xl mb-1">{cat.icon}</div>
                <div className="text-lg font-black">{count}</div>
                <div className="text-[8px] font-black uppercase tracking-wide leading-tight mt-0.5">{cat.label}</div>
              </div>
            );
          })}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col justify-between">
          <div>
            <h3 className="font-black text-slate-900 text-xs uppercase tracking-wider mb-5 flex items-center justify-between">
              <span>Performance par Structure</span><Landmark size={14} className="text-slate-400" />
            </h3>
            <div className="space-y-6">
              {[
                { label: 'Cabinet DOUKE', value: stats.doukeProgress, color: 'from-blue-500 to-blue-600', badge: 'bg-blue-50 text-blue-600' },
                { label: 'CONACCE Chaplains', value: stats.conacceProgress, color: 'from-emerald-500 to-emerald-600', badge: 'bg-emerald-50 text-emerald-600' },
              ].map((s, i) => (
                <div key={i}>
                  <div className="flex justify-between items-center mb-2">
                    <p className="font-black text-slate-700 text-xs uppercase tracking-wide">{s.label}</p>
                    <span className={`font-black text-xs px-2 py-0.5 rounded ${s.badge}`}>{s.value}%</span>
                  </div>
                  <div className="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden border border-slate-200/40">
                    <div className={`bg-gradient-to-r ${s.color} h-full transition-all duration-500`} style={{ width: `${s.value}%` }}></div>
                  </div>
                </div>
              ))}
            </div>
          </div>
          <div className="text-[10px] text-slate-400 font-bold bg-slate-50 p-3 rounded-xl border border-slate-100 mt-6 uppercase tracking-wider text-center">Consolidation en temps réel</div>
        </div>

        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
          <h3 className="font-black text-slate-900 text-xs uppercase tracking-wider mb-4 flex items-center justify-between">
            <span>Avancement par Collaborateur</span><Users size={14} className="text-slate-400" />
          </h3>
          <div className="space-y-3.5">
            {collaborators.map((c, idx) => {
              const colorClass = c.performance > 75 ? 'from-emerald-400 to-emerald-500' : c.performance > 55 ? 'from-blue-500 to-blue-600' : 'from-amber-400 to-amber-500';
              const textBg = c.performance > 75 ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : c.performance > 55 ? 'bg-blue-50 text-blue-700 border-blue-100' : 'bg-amber-50 text-amber-700 border-amber-100';
              const senior = collaborators.find((s) => s.id === c.senior_id);
              const myRealCount = realisations.filter((r) => r.user_id === c.id).length;
              return (
                <div key={idx} className="flex items-center gap-3.5 bg-slate-50/60 p-3 rounded-xl border border-slate-100 hover:bg-slate-50 transition-all">
                  <span className="text-xl bg-white p-2 rounded-xl shadow-sm border border-slate-100 flex items-center justify-center w-10 h-10">{c.avatar_emoji || '👤'}</span>
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-1">
                      <div>
                        <p className="text-xs font-black text-slate-900 leading-none">{c.first_name} {c.last_name}</p>
                        <div className="flex items-center gap-1 mt-0.5">
                          <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wide">{c.role}</p>
                          {senior && <span className="text-[9px] text-blue-500 font-bold">→ {senior.first_name}</span>}
                          <span className="text-[9px] text-slate-400 font-bold">· {myRealCount} réal.</span>
                        </div>
                      </div>
                      <span className={`text-[10px] font-black px-2 py-0.5 rounded border ${textBg}`}>{c.performance}%</span>
                    </div>
                    <div className="w-full bg-slate-200 h-1.5 rounded-full overflow-hidden">
                      <div className={`h-full bg-gradient-to-r ${colorClass} transition-all duration-500`} style={{ width: `${c.performance}%` }}></div>
                    </div>
                  </div>
                </div>
              );
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col justify-between">
          <div>
            <h3 className="font-black text-slate-900 text-xs uppercase tracking-wider mb-4 flex items-center justify-between">
              <span>Anomalies et Blocages</span><AlertCircle size={14} className="text-rose-500" />
            </h3>
            <div className="space-y-3 max-h-[220px] overflow-y-auto pr-1">
              {objectives.filter((o) => o.status === 'En retard').length === 0 ? (
                <div className="text-center py-12 text-slate-400 italic text-xs font-medium">Aucun retard détecté.</div>
              ) : (
                objectives.filter((o) => o.status === 'En retard').map((urg, idx) => (
                  <div key={idx} className="text-xs flex justify-between items-center bg-rose-50/30 p-2.5 rounded-xl border border-rose-100/50">
                    <div>
                      <p className="font-black text-slate-900">{urg.title}</p>
                      <p className="text-[10px] font-bold text-blue-600 uppercase mt-0.5">{urg.structure_type}</p>
                    </div>
                    <span className="font-black text-rose-600 bg-rose-50 px-2.5 py-1 rounded-lg border border-rose-100 text-[10px] shrink-0">{urg.deadline}</span>
                  </div>
                ))
              )}
            </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-black text-slate-900 text-xs uppercase tracking-wider">Traçabilité de Gouvernance (Audit Trail)</h3>
  ...
  return (
    <>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
        {[
          { label: 'Mes Dossiers', value: myObjectives.length, icon: '📁', color: 'text-blue-600', bg: 'bg-blue-50 border-blue-100' },
          { label: 'Avg. Avancement', value: `${myObjectives.length > 0 ? Math.round(myObjectives.reduce((s, o) => s + o.progress_percentage, 0) / myObjectives.length) : 0}%`, icon: '📊', color: 'text-emerald-600', bg: 'bg-emerald-50 border-emerald-100' },
          { label: 'Urgences', value: urgents.length, icon: '⚠️', color: 'text-rose-600', bg: 'bg-rose-50 border-rose-100' },
          { label: 'Juniors suivis', value: myJuniors.length, icon: '👥', color: 'text-purple-600', bg: 'bg-purple-50 border-purple-100' },
        ].map((k, i) => (
          <div key={i} className={`p-5 rounded-2xl border ${k.bg} flex flex-col gap-2`}>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          ...
          <span className="text-[10px] font-black text-slate-400 bg-slate-50 border px-2 py-0.5 rounded">{myObjectives.length} dossiers</span>
        </div>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      <span className={`text-[9px] font-black px-2 py-0.5 rounded border flex items-center gap-1 ${CATEGORY_STYLES[obj.category] || ''}`}>
                        <span>{catInfo?.icon}</span><span>{catInfo?.label}</span>
                      </span>
                      <span className={`text-[9px] font-black px-2 py-0.5 rounded border ${obj.status === 'Terminé' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : obj.status === 'En retard' ? 'bg-rose-50 text-rose-600 border-rose-100' : 'bg-amber-50 text-amber-600 border-amber-100'}`}>{obj.status}</span>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs font-black text-slate-700">{obj.progress_percentage}%</span>
                      <span className="text-[10px] text-slate-400 font-bold">Échéance : {formatDate(obj.deadline)}</span>
                    </div>
                  </div>
                  <span className="text-[10px] font-bold text-slate-400 shrink-0">{myReal.length} réal.</span>
                </div>
              </div>
            );
          })}
      ...
      {myJuniors.length > 0 && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <h3 className="font-black text-slate-900 text-xs uppercase tracking-wider">Collaborateurs Juniors sous ma supervision</h3>
            <span className="text-[10px] font-black text-slate-400 bg-slate-50 border px-2 py-0.5 rounded">{myJuniors.length} junior(s)</span>
          </div>
              ...
              return (
                <div key={ji} className="bg-slate-50 rounded-2xl border border-slate-100 p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <span className="text-xl bg-white p-2 rounded-xl border border-slate-100 shadow-sm">{junior.avatar_emoji}</span>
                      <div>
                        <p className="font-black text-slate-900 text-sm">{junior.first_name} {junior.last_name}</p>
                        <p className="text-[10px] text-slate-500 font-bold uppercase">{junior.role}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-lg font-black text-blue-600">{juniorAvg}%</p>
                      <p className="text-[9px] text-slate-400 font-bold uppercase">Avancement moy.</p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    {juniorObjs.slice(0, 3).map((o, oi) => (
                      <div key={oi} className="flex items-center justify-between bg-white rounded-xl p-2.5 border border-slate-100">
                        <span className="text-xs font-bold text-slate-700 truncate flex-1">{o.title}</span>
                        <div className="flex items-center gap-2 shrink-0 ml-2">
                          <div className="w-16 bg-slate-100 h-1.5 rounded-full overflow-hidden">
                            <div className="bg-gradient-to-r from-blue-500 to-blue-600 h-full" style={{ width: `${o.progress_percentage}%` }}></div>
                          </div>
                          <span className="text-[10px] font-black text-slate-600">{o.progress_percentage}%</span>
                        </div>
                      </div>
                    ))}
                    {juniorObjs.length > 3 && <p className="text-[10px] text-slate-400 font-bold text-center">+{juniorObjs.length - 3} autre(s) dossier(s)</p>}
                  </div>
                  <div className="flex justify-between items-center mt-3 pt-2 border-t border-slate-100">
                    <span className="text-[10px] font-bold text-slate-400">{juniorReal.length} réalisations enregistrées</span>
                    <span className={`text-[9px] font-black px-2 py-0.5 rounded ${junior.performance > 75 ? 'bg-emerald-50 text-emerald-600' : junior.performance > 50 ? 'bg-blue-50 text-blue-600' : 'bg-amber-50 text-amber-600'}`}>Perf. {junior.performance}%</span>
                  </div>
                </div>
              );
            })}
            ...
            {realisations.filter((r) => myJuniors.some((j) => j.id === r.user_id)).slice(0, 5).map((r, ri) => {
              ...
              return (
                <div key={ri} className="p-4 flex items-start gap-3">
                  <span className="text-lg mt-0.5">{collab?.avatar_emoji || '👤'}</span>
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <p className="text-xs font-black text-slate-900">{collab?.first_name} {collab?.last_name}</p>
                      <span className="text-[10px] text-slate-400 font-bold shrink-0">{formatDate(r.date)}</span>
                    </div>
                    <p className="text-xs text-slate-600 font-medium mt-0.5">{r.description}</p>
                    {obj && <p className="text-[10px] text-blue-600 font-bold mt-1">↳ {obj.title} — {r.progress_after}%</p>}
                  </div>
                </div>
              );
            })}
  ...
  return (
    <>
      <div className="bg-gradient-to-r from-[#0F172A] to-[#1E293B] rounded-2xl p-6 text-white border border-slate-700/50 shadow-xl">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <span className="text-4xl bg-white/10 p-3 rounded-2xl border border-white/10">{currentUser.emoji}</span>
            <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Mes Dossiers', value: myObjectives.length, icon: '📁', color: 'text-blue-600', bg: 'bg-blue-50 border-blue-100' },
          { label: 'Prévisions Semaine', value: thisWeekObjs.length, icon: '📅', color: 'text-purple-600', bg: 'bg-purple-50 border-purple-100' },
          { label: 'Réalisations', value: myRealisations.length, icon: '✅', color: 'text-emerald-600', bg: 'bg-emerald-50 border-emerald-100' },
          { label: 'Remarques Reçues', value: myRemarques.length, icon: '💬', color: 'text-amber-600', bg: 'bg-amber-50 border-amber-100' },
        ].map((k, i) => (
          <div key={i} className={`p-5 rounded-2xl border ${k.bg} flex flex-col gap-2`}>
            <span className="text-2xl">{k.icon}</span>
            <p className={`text-2xl font-black ${k.color}`}>{k.value}</p>
      ...
      {thisWeekObjs.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5">
          <h3 className="font-black text-amber-800 text-xs uppercase tracking-wider mb-3 flex items-center gap-2">
            <CalendarRange size={14} /> Prévisions de la semaine — Alertes délais
          </h3>
          <div className="space-y-2">
            {thisWeekObjs.map((o, i) => {
              const daysLeft = Math.ceil((new Date(o.deadline).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
              return (
                <div key={i} className="flex items-center justify-between bg-white rounded-xl p-3 border border-amber-100">
                  <div className="flex-1">
                    <p className="font-black text-slate-900 text-xs">{o.title}</p>
                    <div className="flex items-center gap-2 mt-1">
                      <div className="w-20 bg-slate-100 h-1.5 rounded-full overflow-hidden">
                        <div className="bg-gradient-to-r from-amber-400 to-amber-500 h-full" style={{ width: `${o.progress_percentage}%` }}></div>
                      </div>
                      <span className="text-[10px] font-black text-slate-600">{o.progress_percentage}%</span>
                    </div>
                  </div>
                  <span className={`text-[10px] font-black px-2 py-1 rounded-lg border shrink-0 ml-3 ${daysLeft <= 1 ? 'bg-rose-100 text-rose-700 border-rose-200' : daysLeft <= 3 ? 'bg-amber-100 text-amber-700 border-amber-200' : 'bg-emerald-100 text-emerald-700 border-emerald-200'}`}>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <h3 className="font-black text-slate-900 text-xs uppercase tracking-wider">Mes Objectifs à Atteindre</h3>
          <button onClick={onSaisirRealisation} className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-xl font-black text-[10px] uppercase tracking-wider transition-all flex items-center gap-1.5 shadow-sm">
            <PenLine size={12} /> Saisir une réalisation
          </button>
        </div>
        <div className="divide-y divide-slate-50">
          {myObjectives.length === 0 && <div className="p-8 text-center text-slate-400 text-xs italic">Aucun dossier vous est assigné pour l'instant.</div>}
          {myObjectives.map((obj, i) => {
            const catInfo = getCategoryInfo(obj.category);
            const objReal = myRealisations.filter((r) => r.objective_id === obj.id);
            const objRemarques = myRemarques.filter((r) => r.objective_id === obj.id);
            return (
              <div key={i} className="p-5">
                <div className="flex items-start justify-between gap-4 mb-3">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-2xl font-black text-slate-900">{obj.progress_percentage}%</p>
                    <p className="text-[9px] text-slate-400 font-bold uppercase">avancement</p>
                  </div>
                </div>
                <div className="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden border border-slate-200/50">
                  <div className={`h-full transition-all duration-700 ${obj.progress_percentage === 100 ? 'bg-gradient-to-r from-emerald-500 to-emerald-600' : obj.progress_percentage < 40 ? 'bg-gradient-to-r from-rose-500 to-rose-600' : 'bg-gradient-to-r from-blue-500 to-blue-600'}`}
                    style={{ width: `${obj.progress_percentage}%` }}></div>
                </div>
                {objRemarques.length > 0 && (
                  <div className="mt-3 space-y-1.5">
                    {objRemarques.map((rem, ri) => {
                      const fromUser = collaborators.find((c) => c.id === rem.from_id);
                      return (
                        <div key={ri} className="flex items-start gap-2 bg-blue-50 border border-blue-100 rounded-xl p-2.5">
                          <MessageSquare size={12} className="text-blue-500 mt-0.5 shrink-0" />
                          <div>
                            <p className="text-[9px] font-black text-blue-600 uppercase">{fromUser?.first_name} {fromUser?.last_name} · {formatDate(rem.date)}</p>
                            <p className="text-[11px] text-slate-700 font-medium mt-0.5">{rem.text}</p>
                          </div>
                        </div>
                      );
                    })}
                ...
                {objReal.length > 0 && <p className="text-[10px] text-emerald-600 font-bold mt-2">✅ {objReal.length} réalisation(s) enregistrée(s)</p>}
              </div>
            );
          })}
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <h3 className="font-black text-slate-900 text-xs uppercase tracking-wider">Mes 10 Dernières Réalisations</h3>
          <span className="text-[10px] font-black text-slate-400 bg-slate-50 border px-2 py-0.5 rounded">{myRealisations.length} au total</span>
        </div>
        <div className="divide-y divide-slate-50">
          {myRealisations.length === 0 && <div className="p-8 text-center text-slate-400 text-xs italic">Aucune réalisation enregistrée. Commencez à journaliser vos actions.</div>}
          {myRealisations.slice(0, 10).map((r, ri) => {
            const obj = objectives.find((o) => o.id === r.objective_id);
            return (
              <div key={ri} className="p-4 flex items-start gap-3">
                <div className="bg-emerald-50 border border-emerald-100 p-2 rounded-xl shrink-0"><CheckSquare size={14} className="text-emerald-600" /></div>
                <div className="flex-1">
                  <div className="flex justify-between items-start">
                    <p className="text-xs font-bold text-slate-700">{r.description}</p>
                    <span className="text-[10px] text-slate-400 font-bold shrink-0 ml-2">{formatDate(r.date)}</span>
                  </div>
                  {obj && <p className="text-[10px] text-blue-600 font-bold mt-0.5">↳ {obj.title}</p>}
                  <div className="flex items-center gap-3 mt-1">
                    <span className="text-[10px] text-slate-400 font-bold flex items-center gap-1"><Clock size={9} />{r.duration_hours}h</span>
                    <span className="text-[10px] font-black text-emerald-600">{r.progress_after}%</span>
                    {r.validated_by && <span className="text-[9px] bg-emerald-50 border border-emerald-100 text-emerald-600 font-black px-1.5 py-0.5 rounded uppercase">Validé</span>}
                  </div>
                </div>
              </div>
            );
...
function RealisationsView({
  ...
  const displayReal = (() => {
    if (currentUser.role === 'Super-Admin') return realisations;
    if (isSenior) {
      const myIds = [currentUser.collaborator_id, ...myJuniors.map((j) => j.id)];
      return realisations.filter((r) => myIds.includes(r.user_id));
    }
    return realisations.filter((r) => r.user_id === currentUser.collaborator_id);
  })();
  ...
  return (
    <div className="space-y-5">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="font-black text-slate-900 text-sm uppercase tracking-wider">Journal des Réalisations</h3>
          <p className="text-[11px] text-slate-400 font-medium mt-0.5">{displayReal.length} action(s) enregistrée(s)</p>
        </div>
          ...
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="bg-slate-50 text-slate-400 uppercase text-[10px] tracking-widest border-b border-slate-200">
                ...
                {!isJuniorOrSecretary && <th className="p-4 font-black">Collaborateur</th>}
              ...
              {displayReal.map((r, i) => {
                ...
                return (
                  <tr key={i} className="border-b last:border-0 hover:bg-slate-50/80 transition-all">
                    <td className="p-4 font-bold text-slate-600">{formatDate(r.date)}</td>
                    {!isJuniorOrSecretary && (
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <span className="text-base">{collab?.avatar_emoji}</span>
                          <span className="font-black text-slate-900">{collab?.first_name} {collab?.last_name}</span>
                        </div>
                      </td>
                    )}
                    <td className="p-4 max-w-xs">
                      {obj ? (
                        <div>
                          <span className={`text-[9px] font-black px-1.5 py-0.5 rounded border mb-1 flex items-center gap-1 w-fit ${CATEGORY_STYLES[obj.category] || ''}`}>
                            <span>{catInfo?.icon}</span><span>{catInfo?.label}</span>
                          </span>
                          <p className="text-slate-800 font-bold truncate">{obj.title}</p>
                        </div>
...
function ObjectifsView({
  ...
  onUpdateProgress: (id: string, current: number, title: string, increment: number) => void;
    ...
    <div className="space-y-5">
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-wrap gap-3 items-center justify-between">
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative">
            ...
            <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Rechercher un dossier..."
          ...
          <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-200 rounded-xl px-2 py-1.5">
            ...
            <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)}
              ...
              {ALL_CATEGORIES.map((cat) => <option key={cat.id} value={cat.id}>{cat.icon} {cat.label}</option>)}
            </select>
          </div>
        </div>
        {canAdd && (
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden">
        <div className="p-6 bg-slate-900 text-white flex justify-between items-center">
          <div>
            <h3 className="font-black text-sm uppercase tracking-wider">Console de Gestion des Dossiers</h3>
            <p className="text-[11px] text-slate-400 font-medium mt-0.5">Ajustement dynamique de performance — Sécurisé</p>
          </div>
          <span className="text-[10px] font-black tracking-widest uppercase bg-blue-600 text-white px-3 py-1 rounded-lg">{displayObjectives.length} dossier(s)</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="bg-slate-50 text-slate-400 uppercase text-[10px] tracking-widest border-b border-slate-200">
                ...
                {canEdit && <th className="p-4 font-black text-right">Actions</th>}
              </tr>
            </thead>
            <tbody>
                  ...
                  <tr key={i} className="border-b last:border-0 hover:bg-slate-50/80 transition-all">
                    ...
                    <td className="p-4">
                      <span className={`px-2 py-0.5 rounded-lg font-black text-[10px] border flex items-center gap-1 w-fit ${CATEGORY_STYLES[obj.category] || CATEGORY_STYLES.other}`}>
                        ...
                        <span className="hidden lg:inline">{catInfo?.label}</span>
                      </span>
                    </td>
                    <td className="p-4">
                      <span className={`px-2.5 py-0.5 rounded-full font-black text-[10px] border ${obj.organization_id === 'd2222222-2222-2222-2222-222222222222' ? 'bg-blue-50 text-blue-700 border-blue-100' : 'bg-emerald-50 text-emerald-700 border-emerald-100'}`}>
                        {obj.structure_type}
                      </span>
                    </td>
                    <td className="p-4 font-bold text-slate-600">{formatDate(obj.deadline)}</td>
                    <td className="p-4">
                      <span className={`font-bold ${obj.priority === 'Critique' ? 'text-rose-600 font-black' : obj.priority === 'Haute' ? 'text-amber-600' : 'text-slate-500'}`}>⚡ {obj.priority}</span>
                    </td>
                    <td className="p-4 text-center">
                      <span className={`px-2 py-0.5 rounded-md font-black text-[10px] border ${obj.status === 'Terminé' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : obj.status === 'En retard' ? 'bg-rose-50 text-rose-600 border-rose-100' : 'bg-amber-50 text-amber-600 border-amber-100'}`}>{obj.status}</span>
                    </td>
                    <td className="p-4 text-center">
                      {assignee ? (
                        <div className="flex items-center gap-1.5 justify-center">
                          <span className="text-sm">{assignee.avatar_emoji}</span>
                          <span className="text-[10px] font-bold text-slate-700">{assignee.first_name}</span>
                        </div>
                      ) : <span className="text-[10px] text-slate-300 font-bold">—</span>}
                    </td>
                    <td className="p-4">
                      <div className="flex flex-col items-center gap-1 min-w-[90px]">
                        <span className="font-black text-slate-900 text-xs">{obj.progress_percentage}%</span>
                        <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden border">
                          <div className="bg-gradient-to-r from-blue-500 to-blue-600 h-full transition-all" style={{ width: `${obj.progress_percentage}%` }}></div>
                        </div>
                      </div>
                    </td>
                    {canEdit && (
                      <td className="p-4 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => onUpdateProgress(obj.id, obj.progress_percentage, obj.title, -10)}
                            className="bg-slate-100 hover:bg-slate-200 font-black text-slate-700 px-2 py-1.5 rounded-lg text-[10px] transition-all border">-10%</button>
                          <button onClick={() => onUpdateProgress(obj.id, obj.progress_percentage, obj.title, 10)}
                            className="bg-blue-600 hover:bg-blue-700 text-white font-black px-2 py-1.5 rounded-lg text-[10px] shadow-sm transition-all">+10%</button>
                          <button onClick={() => onEditObjective(obj)} className="bg-amber-50 hover:bg-amber-100 text-amber-600 p-1.5 rounded-lg border border-amber-100 transition-all"><Edit3 size={12} /></button>
                          <button onClick={() => onDeleteObjective(obj.id)} className="bg-rose-50 hover:bg-rose-100 text-rose-600 p-1.5 rounded-lg border border-rose-100 transition-all"><Trash2 size={12} /></button>
                        </div>
                      </td>
                    )}
                  </tr>
...
function EquipeView({
  ...
  const visibleCollabs = canManageUsers
    ? collaborators
    : isSenior
    ? collaborators.filter((c) => c.id === currentUser.collaborator_id || c.senior_id === currentUser.collaborator_id)
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="font-black text-slate-900 text-sm uppercase tracking-wider">Gestion des Collaborateurs</h3>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <h4 className="font-black text-slate-700 text-[10px] uppercase tracking-wider mb-4 flex items-center gap-2"><Link2 size={12} /> Organigramme & Rattachements</h4>
        <div className="space-y-4">
          {visibleCollabs.filter((c) => !c.senior_id || !visibleCollabs.find((v) => v.id === c.senior_id)).map((senior) => {
            const juniors = visibleCollabs.filter((c) => c.senior_id === senior.id);
            const colorClass = senior.performance > 75 ? 'from-emerald-400 to-emerald-500' : senior.performance > 55 ? 'from-blue-500 to-blue-600' : 'from-amber-400 to-amber-500';
            return (
              <div key={senior.id} className="border border-slate-100 rounded-2xl overflow-hidden">
                <div className="flex items-center gap-4 p-4 bg-slate-50 hover:bg-slate-100 transition-all">
                  <span className="text-2xl bg-white p-2 rounded-xl shadow-sm border border-slate-100 flex items-center justify-center w-12 h-12 shrink-0">{senior.avatar_emoji}</span>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="font-black text-slate-900 text-sm">{senior.first_name} {senior.last_name}</p>
                      <span className={`text-[9px] font-black px-2 py-0.5 rounded uppercase tracking-wider border ${senior.profile === 'Super-Admin' ? 'bg-blue-50 text-blue-700 border-blue-100' : senior.profile === 'Senior Analyst' ? 'bg-slate-100 text-slate-700 border-slate-200' : 'bg-emerald-50 text-emerald-700 border-emerald-100'}`}>{senior.profile}</span>
                      {senior.email && <span className="text-[9px] text-slate-400 font-bold">{senior.email}</span>}
                    </div>
                    <p className="text-[10px] text-slate-500 font-bold uppercase mt-0.5">{senior.role}</p>
                    <div className="flex items-center gap-2 mt-1.5">
                      <div className="flex-1 max-w-32 bg-slate-200 h-1.5 rounded-full overflow-hidden">
                        <div className={`h-full bg-gradient-to-r ${colorClass}`} style={{ width: `${senior.performance}%` }}></div>
                      </div>
                      <span className="text-[10px] font-black text-slate-600">{senior.performance}%</span>
                      <span className="text-[9px] text-slate-400 font-bold">{objectives.filter((o) => o.assigned_to === senior.id).length} dossiers</span>
                    </div>
                  </div>
                  {canManageUsers && (
                    <div className="flex gap-2 shrink-0">
                      <button onClick={() => onEditCollaborator(senior)} className="p-2 rounded-lg bg-white border border-slate-200 hover:bg-amber-50 hover:border-amber-200 text-slate-500 hover:text-amber-600 transition-all"><Edit3 size={13} /></button>
                      <button onClick={() => onDeleteCollaborator(senior.id)} className="p-2 rounded-lg bg-white border border-slate-200 hover:bg-rose-50 hover:border-rose-200 text-slate-500 hover:text-rose-600 transition-all"><Trash2 size={13} /></button>
                    </div>
                  )}
                </div>
                {juniors.length > 0 && (
                    ...
                    {juniors.map((junior, ji) => {
                      ...
                      return (
                        <div key={junior.id} className={`flex items-center gap-4 p-3.5 pl-8 hover:bg-slate-50 transition-all ${ji < juniors.length - 1 ? 'border-b border-slate-100' : ''}`}>
                          ...
                          <span className="text-lg bg-white p-1.5 rounded-xl shadow-sm border border-slate-100 flex items-center justify-center w-9 h-9 shrink-0">{junior.avatar_emoji}</span>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                              <p className="font-black text-slate-800 text-xs">{junior.first_name} {junior.last_name}</p>
                              <span className={`text-[9px] font-black px-1.5 py-0.5 rounded uppercase tracking-wider border ${junior.profile === 'Junior' ? 'bg-amber-50 text-amber-700 border-amber-100' : 'bg-teal-50 text-teal-700 border-teal-100'}`}>{junior.profile}</span>
                            </div>
                            <div className="flex items-center gap-2 mt-1">
                          ...
                          {canManageUsers && (
                            <div className="flex gap-2 shrink-0">
                              <button onClick={() => onEditCollaborator(junior)} className="p-1.5 rounded-lg bg-white border border-slate-200 hover:bg-amber-50 hover:border-amber-200 text-slate-400 hover:text-amber-600 transition-all"><Edit3 size={11} /></button>
                              <button onClick={() => onDeleteCollaborator(junior.id)} className="p-1.5 rounded-lg bg-white border border-slate-200 hover:bg-rose-50 hover:border-rose-200 text-slate-400 hover:text-rose-600 transition-all"><Trash2 size={11} /></button>
                            </div>
                          )}
                        </div>
                      );
                ...
                {juniors.length === 0 && <div className="px-8 py-3 text-[10px] text-slate-400 font-bold italic border-t border-slate-50">Aucun junior rattaché.</div>}
              </div>
            );
          })}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {visibleCollabs.map((c, i) => {
          const assignedDossiers = objectives.filter((o) => o.assigned_to === c.id);
          const senior = collaborators.find((s) => s.id === c.senior_id);
          const realCount = realisations.filter((r) => r.user_id === c.id).length;
          return (
            <div key={i} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm text-center relative overflow-hidden flex flex-col justify-between">
              <div>
                <span className="text-4xl block mb-3 bg-slate-50 p-3 rounded-2xl border border-slate-100 w-16 h-16 mx-auto flex items-center justify-center shadow-inner">{c.avatar_emoji || '👤'}</span>
                <h4 className="font-black text-slate-900 text-md">{c.first_name} {c.last_name}</h4>
                <p className="text-[10px] text-blue-600 bg-blue-50 px-2.5 py-0.5 rounded-full font-black uppercase tracking-wider inline-block mt-1.5 border border-blue-100">{c.role}</p>
                {senior && (
                  <div className="mt-2 flex items-center justify-center gap-1 text-[9px] text-slate-400 font-bold">
                    <Link2 size={9} /><span>Réf: {senior.first_name} {senior.last_name}</span>
                  </div>
                )}
              </div>
              <div className="mt-6 pt-4 border-t border-slate-100">
                <div className="grid grid-cols-2 text-left text-xs mb-2">
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-xs font-black text-emerald-500 uppercase tracking-wide">● Actif</span>
                  </div>
                </div>
                <div className="grid grid-cols-2 text-left text-xs border-t border-slate-50 pt-2">
                  <div><span className="text-[9px] text-slate-400 font-bold uppercase">{assignedDossiers.length} dossier(s)</span></div>
                  <div className="text-right"><span className="text-[9px] text-slate-400 font-bold uppercase">{realCount} réal.</span></div>
                </div>
              </div>
              {canManageUsers && (
                <div className="flex gap-2 mt-3">
                  <button onClick={() => onEditCollaborator(c)} className="flex-1 bg-amber-50 hover:bg-amber-100 text-amber-600 py-2 rounded-xl border border-amber-100 font-black text-[10px] uppercase transition-all flex items-center justify-center gap-1">
                    <Edit3 size={11} /> Modifier
                  </button>
                  <button onClick={() => onDeleteCollaborator(c.id)} className="p-2 bg-rose-50 hover:bg-rose-100 text-rose-600 rounded-xl border border-rose-100 transition-all"><Trash2 size={13} /></button>
                </div>
              )}
            </div>
          );
...
export default function FullyLoadedPremiumDashboard() {
  ...
  const [stats, setStats] = useState({ totalObjectives: 0, globalProgress: 0, lateTasks: 0, pendingTasks: 0, conacceProgress: 0, doukeProgress: 0 });
  ...
  const calculateMetrics = (targetObjectives: Objective[]) => {
    ...
    const avgProgress = totalObjs > 0 ? Math.round(targetObjectives.reduce((acc, curr) => acc + curr.progress_percentage, 0) / totalObjs) : 0;
    const late = targetObjectives.filter((o) => o.status === 'En retard').length;
    const pending = targetObjectives.filter((o) => o.status === 'En cours').length;
    ...
    setStats({
      ...
      conacceProgress: conacceObjs.length > 0 ? Math.round(conacceObjs.reduce((a, c) => a + c.progress_percentage, 0) / conacceObjs.length) : 45,
      doukeProgress: doukeObjs.length > 0 ? Math.round(doukeObjs.reduce((a, c) => a + c.progress_percentage, 0) / doukeObjs.length) : 62,
    });
  };

  const loadDataEngine = (user: AuthAccount | null = currentUser) => {
    setLoading(true);
    try {
      const filteredObjs = user && user.orgId !== 'Tous'
        ? BACKUP_OBJECTIVES.filter((o) => o.organization_id === user.orgId)
        : [...BACKUP_OBJECTIVES];
  ...
  useEffect(() => {
    if (isAuthenticated && currentUser) loadDataEngine(currentUser);
  }, [currentUser, isAuthenticated]);

  // ---- ACTIONS OBJECTIFS ----
  // ✅ CORRECTION : on utilise un callback fonctionnel pour éviter les stale closures
  const handleUpdateProgress = (id: string, currentProgress: number, currentTitle: string, increment: number) => {
    ...
    if (currentUser) {
      setActivities((prev) => [{ id: Date.now().toString(), user_id: currentUser.collaborator_id, description: `${currentUser.name} a modifié "${currentTitle}" à ${newProgress}%`, date: todayISO(), type: 'update' }, ...prev]);
    }
  };
  ...
  const handleSaveObjective = (formData: Objective) => {
    ...
    setObjectives((prev) => {
      const updated = isEdit
        ...
        : [{ ...formData, organization_id: currentUser?.orgId !== 'Tous' ? (currentUser?.orgId ?? formData.organization_id) : formData.organization_id }, ...prev];
      calculateMetrics(updated);
      return updated;
    });
    if (currentUser) {
      setActivities((prev) => [{ id: Date.now().toString(), user_id: currentUser.collaborator_id, description: `${currentUser.name} ${isEdit ? 'a modifié' : 'a ouvert'} le dossier "${formData.title}"`, date: todayISO(), type: isEdit ? 'update' : 'creation' }, ...prev]);
    }
  ...
  const handleDeleteObjective = (id: string) => {
    ...
    if (obj && currentUser) {
      setActivities((prev) => [{ id: Date.now().toString(), user_id: currentUser.collaborator_id, description: `${currentUser.name} a supprimé le dossier "${obj.title}"`, date: todayISO(), type: 'deletion' }, ...prev]);
    }
  };
  ...
  const handleSaveCollaborator = (formData: Collaborator) => {
    ...
    if (currentUser) {
      setActivities((prev) => [{ id: Date.now().toString(), user_id: currentUser.collaborator_id, description: `${currentUser.name} ${isEdit ? 'a modifié' : 'a créé'} le collaborateur ${formData.first_name} ${formData.last_name}`, date: todayISO(), type: isEdit ? 'update' : 'creation' }, ...prev]);
    }
  ...
  const handleDeleteCollaborator = (id: string) => {
    ...
    if (collab && currentUser) {
      setActivities((prev) => [{ id: Date.now().toString(), user_id: currentUser.collaborator_id, description: `${currentUser.name} a supprimé le collaborateur ${collab.first_name} ${collab.last_name}`, date: todayISO(), type: 'deletion' }, ...prev]);
    }
  };

  // ---- ACTIONS RÉALISATIONS ----
  const handleSaveRealisation = (formData: Realisation) => {
    // ✅ CORRECTION : on utilise setRealisations puis handleUpdateProgressDirect séparément
    ...
    if (currentUser) {
      ...
      setActivities((prev) => [{ id: Date.now().toString(), user_id: currentUser.collaborator_id, description: `${currentUser.name} a enregistré une réalisation sur "${obj?.title || 'un dossier'}"`, date: todayISO(), type: 'realisation' }, ...prev]);
    }
  };
      ...
      <aside className="w-72 bg-[#0F172A] text-slate-300 flex flex-col shadow-2xl shrink-0 z-10 border-r border-slate-800">
        <div className="flex-1 overflow-y-auto min-h-0">
          ...
          <div className="p-4 mx-4 my-4 bg-[#1E293B]/60 border border-slate-800 rounded-xl shadow-inner">
            ...
            {usingBackupData && <div className="text-[9px] text-amber-400 font-bold uppercase tracking-wider mt-2 flex items-center gap-1">● Mode de Résilience Actif</div>}
          </div>

          <nav className="p-4 space-y-1.5">
            {navItems.map((item, idx) => (
              <button key={idx} onClick={() => setCurrentView(item.view)}
                className={`w-full flex items-center justify-between p-3.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all ${currentView === item.view ? 'bg-gradient-to-r from-blue-600 to-blue-700 text-white shadow-xl shadow-blue-600/20 border-l-4 border-blue-400' : 'hover:bg-slate-800/60 hover:text-white text-slate-400'}`}>
        ...
        <div className="p-4 border-t border-slate-800/80 bg-[#1E293B]/20 flex flex-col gap-2">
          <div className="flex justify-between items-center px-2 text-[10px] font-bold text-slate-500 uppercase tracking-wider">
            <span>Données</span>
            <span className={usingBackupData ? 'text-amber-500' : 'text-emerald-500'}>{usingBackupData ? 'Sécurisées S-Core' : 'Supabase Sync'}</span>
          </div>
          <button onClick={() => loadDataEngine(currentUser)}
      ...
      <main className="flex-1 flex flex-col overflow-y-auto">
        <header className="bg-white border-b border-slate-200 h-20 px-8 flex items-center justify-between shrink-0 shadow-sm">
          <div className="flex items-center gap-6">
            ...
            {currentView === 'Tableau de bord' && isAdmin && (
              <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3 py-1">
                ...
                <select className="bg-transparent font-black text-xs text-slate-700 py-1 outline-none cursor-pointer" onChange={(e) => setCurrentStructure(e.target.value)} value={currentStructure}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="w-11 h-11 bg-gradient-to-tr from-slate-800 to-slate-900 rounded-2xl flex items-center justify-center text-white font-black text-lg shadow-md border border-slate-700">{currentUser.emoji}</div>
          </div>
        </header>
        ...
        <div className="p-8 space-y-8">
          {currentView === 'Tableau de bord' && (
            <>
              {isAdmin && (
                <AdminDashboard objectives={displayObjectives} collaborators={collaborators} activities={activities} realisations={realisations} stats={stats}
                  currentStructure={currentStructure} setCurrentView={setCurrentView} currentUser={currentUser} />
              )}
              {isSenior && (
                <SeniorDashboard objectives={objectives} collaborators={collaborators} realisations={realisations} remarques={remarques} stats={stats} currentUser={currentUser} />
              )}
              {isJuniorOrSecretary && (
                <JuniorDashboard objectives={objectives} collaborators={collaborators} realisations={realisations} remarques={remarques}
                  currentUser={currentUser} onSaisirRealisation={() => setIsRealisationModalOpen(true)} />
              )}
            </>
          )}
          ...
          {currentView === 'Planification' && (
            <div className="bg-white rounded-2xl border p-12 shadow-sm text-center max-w-xl mx-auto my-12">
              ...
              <h4 className="font-black text-slate-900 text-sm uppercase tracking-wider">Planificateur de Livrables de Cabinet</h4>
              <p className="text-xs text-slate-500 mt-2 leading-relaxed">
                Cette console regroupe la frise chronologique interconnectée avec les dates de conseils d'administration du Cabinet DOUKE et les jalons de validation terrain de la CONACCE Chaplains.
              </p>
            </div>
          )}

          {currentView === 'Équipe' && (
            <EquipeView collaborators={collaborators} objectives={objectives} realisations={realisations} currentUser={currentUser}
        ...
        <div className="fixed bottom-8 right-8 flex flex-col gap-3">
          <button onClick={() => setIsRealisationModalOpen(true)} title="Saisir une réalisation"
          ...
          {!isJuniorOrSecretary && (
            <button onClick={() => { setEditingObjective(null); setIsObjectiveModalOpen(true); }} title="Nouveau dossier"
